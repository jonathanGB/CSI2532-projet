...
exports.isSecretaire = (tel, controllerCallback) => {
  ...
  pg.connect(conString, (err, client, done) => {
    ...
    if(err) {
      ...
      console.log('kdflkskdlflsdfjsdlf');
      return controllerCallback([]);
    }
      ...
      if (results.length === 0) {
        ...
        query2.on('end', function() {
          if (results2.length === 0) {
            ...
            var results3 = [];
            // no conflict
            var query3 = client.query('SELECT * FROM Consultation WHERE dateC = $1 AND heureDebut = $2 AND (medecinId = $3 OR SSN = $4)', [userData[4], userData[5], userData[0], userData[2]]);
  ...
  pg.connect(conString, (err, client, done) => {
    ...
    var query;
      ...
      if (dataSent[0] === 'medecinId')
        query = client.query("SELECT * FROM Consultation WHERE medecinId = $1 ORDER BY dateC, heureDebut", [dataSent[1]]);
      else if (dataSent[0] === 'phoneNoS')
        query = client.query("SELECT * FROM Consultation WHERE phoneNoS = $1 ORDER BY dateC, heureDebut", [dataSent[1]]);
      else
        query = client.query("SELECT * FROM Consultation WHERE SSN = $1 ORDER BY dateC, heureDebut", [dataSent[1]]);
...
exports.updateObjet = (dataSent, controllerCallback) => {
  pg.connect(conString, function(err, client, done) {
        ...
        // SQL Query > Insert Data
        client.query('UPDATE Consultation SET objet = $4 WHERE SSN = $1 AND dateC = $2 AND heureDebut = $3', dataSent, (err) => {
          controllerCallback(err);
          done();
        });
    });
...
exports.getConsultationPrescription = (dataSent, controllerCallback) => {
  ...
  pg.connect(conString, (err, client, done) => {
    ...
    var query = client.query("SELECT nomExam FROM PrescriptionES WHERE SSN = $1 AND dateC = $2 AND heureDebut = $3 ORDER BY nomExam", dataSent);
    ...
    // After all data is returned, close connection and return results
    query.on('end', function() {
      var query2 = client.query("SELECT nom, dureeValidite FROM PrescriptionM WHERE SSN = $1 AND dateC = $2 AND heureDebut = $3 ORDER BY nom", dataSent);
